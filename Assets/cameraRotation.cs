...
public class cameraRotation : MonoBehaviour {
	...
	void LateUpdate () {
        ...
        xrot = Mathf.Clamp(Input.GetAxis("CameraY") * Time.deltaTime * 20 * YSENSITIVITY + xrot, -90, 90);
        lookPos = Vector3.Lerp(maxZ.position, minZ.position, (xrot+90)/180);
        StatePatternController.SlerpForMe(transform, new Vector3(lookPos.x,lookPos.y, lookPos.z), dampSpeed, Time.deltaTime);
